...
# Estilo visual Verde Oliva (Identidade visual aproximada)
st.markdown("""
    <style>
...
with st.expander("SECAO 1 – DADOS INICIAIS", expanded=True):
    ...
    # Lista completa de areas citadas no documento
...
secoes_dados = {
    ...
    "SECAO 12 – FORMATO DE ATUAÇÃO": ["O formato (remoto/presencial) impacta a comunicacao", "Recebo informacoes suficientes mesmo a distancia"]
}
...
# --- BOTAO DE ENVIO E INTEGRACAO ---
if st.button("ENVIAR"):
    # URL gerada pelo Power Automate (Substitua pela sua URL entre as aspas)
    ...
    if URL_WEBHOOK == "COLE_SUA_URL_AQUI":
        ...
        # Gerando ID unico baseado no timestamp do envio
        id_formulario = datetime.now().strftime("%Y%m%d%H%M%S")
        data_hora_envio = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
        
        erro_no_loop = False

        # Loop para enviar cada resposta individualmente (Base Vertical)
        for pergunta_texto, nota_valor in respostas_finais.items():
            payload = {
                "ID": id_formulario,
                "Genero": genero,
                "Setor": setor,
            ...
            try:
                # Envio para o Webhook do Power Automate
                envio_servidor = requests.post(URL_WEBHOOK, json=payload)
                if envio_servidor.status_code not in [200, 202]:
                    erro_no_loop = True
            except:
                erro_no_loop = True
        ...
        if not erro_no_loop:
            st.success("Resposta enviada com sucesso! Agradecemos sua colaboracao")
        else:
            st.error("Ocorreu um problema ao enviar sua resposta. Por favor verifique sua conexao")
